...
constexpr auto getColorCode(const char color) noexcept {
    switch (color) {
        case 'r': // Red
            return "\033[1;31m";
        case 'g': // Green
...
void printColoredError(const char color, std::format_string<Args...> fmt, Args &&... args) {
    ...
    else std::print(std::cerr, "{}{}\033[0m", getColorCode(color), std::format(fmt, std::forward<Args>(args)...));
}
...
void printColoredErrorln(const char color, std::format_string<Args...> fmt, Args &&... args) {
    ...
    else std::print(std::cerr, "{}{}\033[0m\n", getColorCode(color), std::format(fmt, std::forward<Args>(args)...));
}
